...
class PerchTemplatedForm
{
    ...
    private function _replace_select_field($match, $Tag, $template)
    {
        ...
        $value = $attrs['value'];  // preserve this - never overwrite it
    
        if ($Tag->allowempty()) {
            $s = '';
            if ($Tag->placeholder()) $s .= $Tag->placeholder().'|';
            array_unshift($opts, $s);
        } else {
            if ($Tag->placeholder()) {
                array_unshift($opts, '!'.$Tag->placeholder().'|');
            }
        }
        ...
        if (PerchUtil::count($opts)) {
            foreach($opts as $opt) {
                ...
                if (strpos($opt, '|')) {
                    $parts = explode('|', $opt);
                    $text = $parts[0];
                    $val  = $parts[1] !== '' ? $parts[1] : $parts[0];
                }
    
                if (is_array($value)) {
                    if (in_array(trim($val), $value)) $attrs['selected'] = 'selected';
                } else {
                    if (trim($val) == $value) $attrs['selected'] = 'selected';
                }
    
                $attrs['value'] = trim($val);  // only set on $attrs, never back to $value
