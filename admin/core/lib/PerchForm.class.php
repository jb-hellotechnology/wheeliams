...
class PerchForm
{
	...
	{
		$name = $multiple ? $id . '[]' : $id;  // add this
	
		$this->fields[] = $id;
	
		$opener = '<select id="'.$this->html($id, true).'" name="'.$this->html($name, true).'" class="'.$this->html($class, true).'"'.($multiple ? ' multiple' : '').($attributes ? ' '.$attributes : '').'>';
